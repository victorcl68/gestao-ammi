...
Site simples (HTML/CSS/JS puro, sem build, mobile-first) para dois controles financeiros manuais de uma loja:
...
Em telas largas (desktop), as duas seções aparecem lado a lado na mesma página. Em celular, cada uma é uma tela separada acessada por um menu inicial.
...
2. Abra o **SQL Editor** do projeto e rode o conteúdo de [schema.sql](schema.sql) — isso cria as tabelas `caixa_casa_lancamentos` e `salario_lancamentos`, os índices e as políticas de Row Level Security (RLS).
...
Tudo consolidado em poucos arquivos na raiz:

```
index.html   - as 4 telas (login, home, caixa casa, salário), alternadas via JS ([hidden])
style.css    - todos os estilos (mobile-first, com layout lado a lado em telas largas)
app.js       - client Supabase, autenticação, navegação e lógica das duas seções
schema.sql   - tabelas, índices e políticas de RLS do Supabase
ammi-logo.png - logo usada como favicon e na tela de login
```

## Próximos passos (fora do escopo atual)

- Um novo módulo (ex: "contas a pagar") pode seguir o mesmo padrão: uma nova tabela `*_lancamentos` e uma nova seção na tela, sem alterar o que já existe.
